...
export async function submitReportCore(
  db: Firestore,
  input: { uid: string; idempotencyKey: string; report: ReportInput },
  now: Date = new Date(),
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (existing.exists) {
      ...
      // Replaying someone else's key would hand back their reference code —
      // and that code is the whole credential, the one thing that opens their
      // case. The key is generated by the client and travels in the clear, so
      // it was never a secret and cannot be treated as one. Only the device
      // that filed a report may learn what came of it.
      if (data.uid !== input.uid) {
        logProblem({ event: "submit_idempotency_uid_mismatch" });
    ...
    tx.set(idempotencyRef, {
      // Whose submission this was. Checked on every replay above.
      uid: input.uid,
      reportId: reportRef.id,
      referenceCode,
      createdAt,
...
export const submitReport = onCall(
  ...
  async (request): Promise<{ referenceCode: string }> => {
    ...
    const result = await submitReportCore(db, { uid, ...parsed.data });
